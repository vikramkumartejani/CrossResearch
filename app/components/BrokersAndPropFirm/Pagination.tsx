...
export default function Pagination({
    ...
    return (
        <div className="flex flex-col gap-4">
            {/* Mobile: pagination buttons centered */}
            <div className="flex sm:hidden items-center justify-center gap-3">
                <button onClick={() => onPageChange(currentPage - 1)} disabled={currentPage === 1}
                    className="w-8 h-8 cursor-pointer flex items-center justify-center rounded-full bg-[#FFFFFF08] hover:text-white disabled:opacity-50 disabled:cursor-not-allowed transition-colors" aria-label="Previous page">
                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M10.5 13L5.5 8L10.5 3" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>
                </button>
                <div className="flex items-center gap-1">
                    {pages.map((p, i) => p === '...' ? (
                        <span key={`e-${i}`} className="mx-1 w-8 bg-[#FFFFFF08] rounded-full h-8 flex items-center justify-center text-white/80 text-[16px] font-semibold">…</span>
                    ) : (
                        <button key={p} onClick={() => onPageChange(p as number)}
                            className={`w-8 h-8 cursor-pointer flex items-center justify-center rounded-full text-[16px] font-semibold transition-colors ${currentPage === p ? 'bg-white text-[#070711]' : 'text-white/80 bg-[#FFFFFF08]'}`}>
                            {p}
                        </button>
                    ))}
                </div>
                <button onClick={() => onPageChange(currentPage + 1)} disabled={currentPage === totalPages}
                    className="w-8 h-8 cursor-pointer flex items-center justify-center rounded-full bg-[#FFFFFF08] disabled:opacity-50 disabled:cursor-not-allowed transition-colors" aria-label="Next page">
                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M5.5 13L10.5 8L5.5 3" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>
                </button>
            </div>

            {/* Mobile: Result + Pages in one row on top | Desktop: all in one row */}
            <div className="flex sm:hidden items-center justify-center gap-8">
                ...
                <div className="relative" ref={dropdownRef}>
                    <button
                        ...
                        className="flex items-center gap-4 bg-[#FFFFFF08] cursor-pointer rounded-[90px] px-5 py-2 text-[14px] leading-4.5 font-semibold text-white/70 transition-colors"
                    >
                        <span className="font-medium text-white/70">{totalPages} Pages</span>
                        <svg width="16" height="16" className={`transition-transform duration-200 ${dropdownOpen ? 'rotate-180' : ''}`} viewBox="0 0 16 16" fill="none">
                    ...
                    {dropdownOpen && onItemsPerPageChange && (
                        <div className="absolute right-0 bottom-full mb-2 w-36 bg-[#1C2437] border border-white/10 rounded-xl overflow-hidden shadow-xl z-[999]">
                            {PAGE_SIZE_OPTIONS.map((size) => (
                                <button key={size} onClick={() => { onItemsPerPageChange(size); onPageChange(1); setDropdownOpen(false); }}
                                    className={`w-full text-left px-4 py-2.5 text-[14px] cursor-pointer transition-colors ${itemsPerPage === size ? 'text-white bg-white/10 font-medium' : 'text-white/60 hover:text-white hover:bg-white/5'}`}>
                                    {size} per page
                                </button>
                            ))}
                        </div>
            ...
            <div className="hidden sm:flex items-center justify-between gap-4">
                ...
                <div className="relative" ref={dropdownRef}>
                    ...
                    {dropdownOpen && onItemsPerPageChange && (
                        <div className="absolute right-0 bottom-full mb-2 w-full bg-[#1C2437] border border-white/10 rounded-xl overflow-hidden shadow-xl z-[999]">
                            {PAGE_SIZE_OPTIONS.map((size) => (
                                <button
                                    key={size}
                                    onClick={() => {
