        ...
        <div className="bg-[#16161F]">
            ...
            <div className="grid grid-cols-[1fr_64px_64px_64px] sm:grid-cols-[1fr_80px_80px_80px] gap-1 sm:gap-4 px-3 sm:px-4 pb-3 border-b border-[#FFFFFF0D]">
                {['Instrument', 'Net', 'Wow', '10W Trend'].map((h) => (
                    <span key={h} className="text-white/60 text-[11px] sm:text-[14px] leading-[17px] font-normal">{h}</span>
                ))}
            </div>
            {/* Rows */}
            <div className="flex flex-col">
                {rows.map((row) => (
                    ...
                    >
                        <div>
                            <p className="text-white text-[12px] sm:text-[14px] leading-[17px] font-semibold mb-1">{row.name}</p>
                            <p className="text-white/60 text-[11px] sm:text-[12px] leading-[14px]">{row.code}</p>
                        </div>
                        <div>
                            <p className={`text-[12px] sm:text-[14px] leading-[17px] font-semibold ${row.netPos ? 'text-[#2CB37B]' : 'text-[#E25C3F]'}`}>
                                {row.net}
                            </p>
                            <p className="text-[11px] sm:text-[12px] leading-[14px] font-normal text-white/60 mt-1">NET</p>
                        </div>
                        <div>
                            <p className={`text-[12px] sm:text-[14px] leading-[17px] font-semibold ${row.wowPos ? 'text-[#2CB37B]' : 'text-[#E25C3F]'}`}>
                                {row.wow}
                            </p>
            ...
            <div className="mt-4 grid grid-cols-1 xl:grid-cols-2 2xl:grid-cols-3 gap-3 sm:gap-4">
                ...
                <div className="bg-[#16161F] flex flex-col">
                    <div className="p-3 sm:p-4 flex flex-col sm:flex-row sm:items-start justify-between gap-3 border-b border-[#FFFFFF0D]">
                        ...
                        <span className="bg-[#FFFFFF0A] rounded-full py-2 px-3 sm:px-4 text-white/60 text-[12px] leading-[14px] font-normal self-start flex-shrink-0">May 24 2025</span>
                    </div>

                    <div className="flex flex-col gap-2.5 sm:gap-4 flex-1 p-3 sm:p-4">
                        {COMMENTARY.map((item) => (
                            <div key={item.num} className="flex gap-2 sm:gap-4">
                                <span className="text-[#88C4FF] text-[13px] sm:text-[14px] leading-[17px] font-semibold flex-shrink-0">{item.num}</span>
                    ...
                    <div className="mt-3 sm:mt-4 p-3 sm:p-4 border-t border-[#FFFFFF0D]">
                        <p className="text-[#88C4FF] text-[14px] sm:text-[16px] leading-[19px] font-semibold uppercase mb-2 sm:mb-3">Regime Takeaway</p>
                        <p className="text-white/60 text-[12px] leading-4 sm:leading-[18px]">{REGIME_TAKEAWAY}</p>
                    </div>
                </div>
            </div>
        </div>
