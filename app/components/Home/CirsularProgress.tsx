...
function buildSVGContent(pct: number): string {
  ...
  if (pct > 0) {
    ...
    if (pct >= 99.9) {
      content += `<circle cx="${CX}" cy="${CY}" r="${(OUTER_R + INNER_R) / 2}" fill="none" stroke="${ACCENT}" strokeWidth="${OUTER_R - INNER_R}"/>`;
    } else {
  ...
  for (let i = 0; i < TOTAL_TICKS; i++) {
    ...
    content += `<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="${isLit ? ACCENT : DIM}" strokeWidth="1.3"/>`;
  }
